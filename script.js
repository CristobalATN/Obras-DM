// ===== CONFIGURATION =====
const POWER_AUTOMATE_URL = 'https://default0c13096209bc40fc8db89d043ff625.1a.environment.api.powerplatform.com:443/powerautomate/automations/direct/workflows/a3454ce8bfbb4c48aef3c62c9000328f/triggers/manual/paths/invoke?api-version=1&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=AFPTHW6zv1zn9nS08ki22cqTTTv0Cn6vl6wVzA009rE'; // Replace with actual URL
...
async function loadJSONData() {
  ...
  try {
    const autoresResponse = await fetch('autores.json');
    if (autoresResponse.ok) {
      ...
      autoresData = autoresRaw.map((autor, index) => ({
        id: autor['Nombre Completo'] || `autor${index}`,
        text: autor['Nombre Completo'] || `Autor ${index}`,
        email: autor['Correo Electrónico'] || null
      }));
    } else {
      console.warn('Could not load autores.json, using default data');
      autoresData = [
        { id: 'Autor 1', text: 'Autor 1', email: null },
        { id: 'Autor 2', text: 'Autor 2', email: null },
        { id: 'Autor 3', text: 'Autor 3', email: null }
      ];
    }
  } catch (error) {
...
function addParticipacion() {
  ...
  $(`.participacion-autor[data-index="${rowIndex}"]`).on('change', function () {
    updateParticipacionData(rowIndex, 'autor', $(this).val());
  });
...
function handleFormSubmit(e) {
  ...
  if (!selectedSignatureFile) {
    showMessage('Debe adjuntar una imagen de firma', 'error');
    isValid = false;
  }
...
async function prepareAndSubmitData() {
  ...
  try {
    ...
    const signatureBase64 = await fileToBase64(selectedSignatureFile);
    ...
    participacionesData.forEach((p, index) => {
      ...
      if (clase && autorNombre && porcentaje > 0) {
        ...
        const autorCorreo = autorData ? autorData.email : null;
    ...
    const formData = {
      ...
      firmaFilename: selectedSignatureFile.name,
